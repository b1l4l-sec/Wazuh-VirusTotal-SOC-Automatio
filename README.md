...
|---------|-------|----------|
| Active response script received empty file path | Wazuh JSON contains unescaped nested JSON in `full_log` that breaks `jq` | Used `grep -oP` regex extraction instead of `jq` |
| Quarantine never triggered despite VirusTotal detecting malware | Rule 100203 was overriding Rule 100201 at a lower severity, preventing the active response trigger | Removed conflicting rule |
